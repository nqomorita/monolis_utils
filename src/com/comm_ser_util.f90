...
contains
  ...
  subroutine monolis_comm_get_send_serial(n_domain, domain_id, n_vertex, vertex_id, com, recv_list)
    ...
    !> [in] recv 節点の情報
    type(monolis_comm_node_list) :: recv_list
    integer(kint) :: i, n_neib, id
    integer(kint), allocatable :: temp(:), local_nid(:)

    call monolis_alloc_I_1d(temp, recv_list%n_node)

    temp = recv_list%domid

    call monolis_get_uniq_array_I(temp, recv_list%n_node, n_neib)
    ...
    if(n_neib == 0)then
      ...
      com%send_neib_pe = temp(1:n_neib)
    endif
