!> @ingroup driver
program monolis_dbc_all_surf_tet_R
  ...
  if(is_get)then
    write(*,"(a)")"usage:"
    write(*,"(a)") &
    & "./monolis_dbc_all_surf_tet_R {options} {num of dof} {value}"
    write(*,"(a)")""
    write(*,"(a)")"options:"
  ...
  call monolis_driver_get_arg_dbc_all_R(n_dof, val, is_get)

  if(.not. is_get)then
    call monolis_std_error_string("input parameters are not set")
    call monolis_std_error_string("./monolis_dbc_all_surf_tet_R {options} {num of dof} {value}")
    stop monolis_fail
  endif
...
end program monolis_dbc_all_surf_tet_R
