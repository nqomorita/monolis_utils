!> @ingroup driver
program monolis_dbc_all_surf_hex_C
  ...
  if(is_get)then
    write(*,"(a)")"usage:"
    write(*,"(a)") &
    & "./monolis_dbc_all_surf_hex_C {options} {num of dof} {value}"
    write(*,"(a)")""
    write(*,"(a)")"options:"
  ...
  if(.not. is_get)then
    ...
    call monolis_std_error_string("./monolis_dbc_all_surf_hex_C {options} {num of dof} {value}")
    stop monolis_fail
  endif
...
end program monolis_dbc_all_surf_hex_C
